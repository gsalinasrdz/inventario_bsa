...
import {
  useDevoluciones, useDevolucion,
  useCrearDevolucion, useAprobarDevolucion, useRechazarDevolucion,
...
type LineaForm = { producto_id: number; nombre: string; sku: string; cantidad: number; precio_unitario: number }
const LINEA_VACIA: LineaForm = { producto_id: 0, nombre: '', sku: '', cantidad: 1, precio_unitario: 0 }

function DevolucionForm({ onSubmit, isLoading, onCancel }: { onSubmit: (d: any) => void; isLoading?: boolean; onCancel: () => void }) {
  ...
  const { data: clientesData }  = useQuery({ queryKey: ['clientes-list'], queryFn: () => apiClient.get('/clientes', { params: { activo: true, per_page: 100 } }).then(r => r.data.data) })
  ...
  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault()
    if (!almacenId) return
    onSubmit({ cliente_id: clienteId || undefined, almacen_id: almacenId, fecha_devolucion: fecha, motivo: motivo || undefined, notas: notas || undefined,
      detalles: lineas.filter(l => l.producto_id).map(l => ({ producto_id: l.producto_id, cantidad: l.cantidad, precio_unitario: l.precio_unitario })) })
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
        <div>
          ...
          <select value={clienteId} onChange={e => setClienteId(Number(e.target.value) || '')}
            ...
            {clientesData?.map((c: any) => <option key={c.id} value={c.id}>{c.nombre}</option>)}
          </select>
        </div>
        <div>
        ...
        <div>
          ...
          <input type="text" value={motivo} onChange={e => setMotivo(e.target.value)} placeholder="Ej. Producto defectuoso"
            className="w-full px-3 py-2 border border-input rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-primary/30" />
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg overflow-hidden">
        <div className="flex items-center justify-between px-3 py-2 bg-gray-50 border-b">
          <span className="text-xs font-semibold text-gray-700">Productos devueltos</span>
            ...
            {lineas.map((l, i) => (
              <tr key={i}>
                ...
                <td className="px-3 py-2"><input type="number" min="0.001" step="0.001" value={l.cantidad} onChange={e => upd(i, 'cantidad', Number(e.target.value))}
                  className="w-20 text-right px-2 py-1 border border-input rounded text-xs focus:outline-none focus:ring-1 focus:ring-primary/30" /></td>
                <td className="px-3 py-2"><input type="number" min="0" step="0.01" value={l.precio_unitario} onChange={e => upd(i, 'precio_unitario', Number(e.target.value))}
                  className="w-24 text-right px-2 py-1 border border-input rounded text-xs focus:outline-none focus:ring-1 focus:ring-primary/30" /></td>
      ...
      <div className="flex justify-end gap-3 pt-4 border-t">
        <button type="button" onClick={onCancel} disabled={isLoading}
          className="px-4 py-2 text-sm border border-input rounded-lg hover:bg-gray-50">Cancelar</button>
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 gap-3 text-sm">
        <div><p className="text-xs text-muted-foreground">Folio</p><p className="font-mono font-medium">{dev.folio}</p></div>
        <div><p className="text-xs text-muted-foreground">Fecha</p><p>{formatDate(dev.fecha_devolucion)}</p></div>
        <div><p className="text-xs text-muted-foreground">Cliente</p><p>{dev.cliente ?? '—'}</p></div>
        <div><p className="text-xs text-muted-foreground">Almacén</p><p>{dev.almacen ?? '—'}</p></div>
        {dev.motivo && <div className="col-span-2"><p className="text-xs text-muted-foreground">Motivo</p><p>{dev.motivo}</p></div>}
      </div>
        ...
        <tbody className="divide-y">
          {dev.detalles?.map((d: any) => (
            <tr key={d.id}>
              ...
              <td className="px-3 py-2 text-right text-xs">{formatCurrency(d.precio_unitario)}</td>
              <td className="px-3 py-2 text-right text-xs font-medium">{formatCurrency(d.subtotal)}</td>
            </tr>
          ))}
        </tbody>
      </table>
...
export default function DevolucionesPage() {
  ...
  const [aprobarTarget, setAprobarTarget] = useState<Devolucion | null>(null)
  const [rechazarTarget, setRechazarTarget] = useState<Devolucion | null>(null)

  const { data, isLoading, refetch } = useDevoluciones({ estado: estadoFilter || undefined, page, per_page: 25 })
  const crearMutation    = useCrearDevolucion()
  const aprobarMutation  = useAprobarDevolucion()
  const rechazarMutation = useRechazarDevolucion()
    ...
    <div className="space-y-5">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-sm text-muted-foreground">{meta ? `${meta.total} devoluciones` : 'Devoluciones de clientes'}</p>
        </div>
      ...
      <div className="bg-white rounded-xl border border-border overflow-hidden">
        {isLoading ? (
          ...
          <div className="flex items-center justify-center py-16 text-muted-foreground"><p>Sin devoluciones</p></div>
        ) : (
          <table className="w-full text-sm">
            ...
            <tbody className="divide-y">
              {items.map((d: Devolucion) => {
                const cfg = ESTADO_CFG[d.estado] ?? ESTADO_CFG.PENDIENTE
                return (
                  <tr key={d.id} className="hover:bg-gray-50/50">
                    <td className="px-4 py-3 font-mono text-xs font-medium">
                      <button onClick={() => setDetailId(d.id)} className="text-primary hover:underline">{d.folio}</button>
                    </td>
                    <td className="px-4 py-3 text-muted-foreground">{d.cliente ?? '—'}</td>
                    <td className="px-4 py-3 text-muted-foreground">{d.almacen ?? '—'}</td>
                    <td className="px-4 py-3 text-muted-foreground">{formatDate(d.fecha_devolucion)}</td>
                    <td className="px-4 py-3 text-right">{d.detalles_count}</td>
                    <td className="px-4 py-3 text-right">
                      <span className={cn('text-xs font-medium px-2 py-0.5 rounded-full', cfg.color)}>{cfg.label}</span>
                    </td>
                    <td className="px-4 py-3">
                      <div className="flex items-center justify-end gap-1">
                        {can('devoluciones.aprobar') && d.estado === 'PENDIENTE' && (<>
                          <button onClick={() => setAprobarTarget(d)} className="p-1.5 text-muted-foreground hover:text-green-600 hover:bg-green-50 rounded" title="Aprobar">
                            <CheckCircle className="w-4 h-4" />
                          </button>
                          <button onClick={() => setRechazarTarget(d)} className="p-1.5 text-muted-foreground hover:text-destructive hover:bg-destructive/10 rounded" title="Rechazar">
      ...
      {crearModal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-3xl mx-4 max-h-[92vh] overflow-y-auto">
            <div className="p-6 border-b sticky top-0 bg-white z-10"><h2 className="text-lg font-semibold">Nueva Devolución</h2></div>
            <div className="p-6">
              <DevolucionForm onSubmit={async (d) => { await crearMutation.mutateAsync(d); setCrearModal(false) }} isLoading={crearMutation.isPending} onCancel={() => setCrearModal(false)} />
            </div>
          </div>
        </div>
      )}
      ...
      {detailId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center">
          ...
          <div className="relative bg-white rounded-xl shadow-xl w-full max-w-2xl mx-4 max-h-[92vh] overflow-y-auto">
            <div className="p-6 border-b sticky top-0 bg-white z-10"><h2 className="text-lg font-semibold">Detalle de Devolución</h2></div>
            <div className="p-6"><DetailModal id={detailId} onClose={() => setDetailId(null)} /></div>
          </div>
        </div>
      )}

      <ConfirmDialog open={!!aprobarTarget} title="Aprobar Devolución"
        description={`¿Aprobar la devolución "${aprobarTarget?.folio}"? El stock será incrementado.`}
      ...
      <ConfirmDialog open={!!rechazarTarget} title="Rechazar Devolución"
        description={`¿Rechazar la devolución "${rechazarTarget?.folio}"?`}
